...
boundaries in Agent Skills `0.5.0`, coordinated with CLI `0.3.0` and SDK
`0.4.0`.
...
- Hand project creation, source preflight, table selection, reconfiguration,
  and lifecycle work to the dashboard without collecting source credentials.
...
- Reject target schema changes, imports, rows, backfills, target database
  credentials, custom capture workers, and non-dashboard sync control in the
...
| CLI guidance does not invent sync commands or expose source credentials. | CLI synced reference | CLI command-boundary test |
...
- CLI `0.3.0` does not contain every local synced-project guard. The Runtime
  server remains authoritative, and the skills avoid prohibited commands before
  dispatch.
