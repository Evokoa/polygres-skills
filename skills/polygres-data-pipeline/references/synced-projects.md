...
new synced project through the dashboard.

## Keep credentials in the dashboard

Direct the user to **New project**, select Supabase, Neon, or Postgres Database,
and enter the source connection in the dashboard. Never request, inspect,
record, or pass the source URL or password through chat, a plan, generated
files, logs, CLI arguments, the Runtime API, or the SDK. A direct PostgreSQL
endpoint is required; do not use a pooler URL, provider API URL, `psql` command,
or publishable key.

Treat the dashboard preflight result as authoritative. It checks supported
PostgreSQL versions, network and TLS access, authentication, logical
replication, publication and replication privileges, slot capacity, storage,
and eligible sync keys. Current managed sync selects eligible tables in the
`public` schema. Polygres creates the filtered publication and replication slot
...
table or column scope. Represent dashboard work with a `dashboard` interface.
