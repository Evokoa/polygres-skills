...
- For a synced project, use the dashboard for creation, preflight, table
  selection, reconfiguration, and lifecycle actions. Use the project Runtime
  API key only for supported graph, text, vector, hybrid, Context, catalog, and
  readiness work. Do not use CLI import, migrations, rows, SDK rows,
  `connection_info()`, or direct target Postgres.
