...
def _synced_project_blockers(
    ...
    if sync_actions and (
        not isinstance(sync_interface, dict) or sync_interface.get("surface") != "dashboard"
    ):
        blockers.append(
            _issue(
                "synced-control-plane-handoff-required",
                "sync.interface",
                "record a dashboard handoff for synced-project control-plane actions",
            )
        )
    ...
    for path, interface in _interfaces(plan):
        ...
        if "sync" in operation and surface != "dashboard":
            blockers.append(
                _issue(
                    "synced-control-plane-handoff-required",
                    path,
                    "create and configure sync through the dashboard, not CLI, SDK, or Runtime",
                )
            )
    return blockers
