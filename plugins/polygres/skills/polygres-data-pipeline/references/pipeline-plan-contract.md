...
selected table or column scope without a connection value. Use `surface:
dashboard` for sync creation and configuration interfaces.
...
6. a synced-project plan selects a target schema mutation, target rows,
   ...
   or non-dashboard sync control interface.
